...
usage ()
{
  ...
  echo -e "  Purpose: Lists all the applications using reportapps \n" >&2; exit 1;
}
...
# echo $session_id
#curl -sS -w "\n" -k https://$ActIP/actifio/api/report/reportapps?sessionid=$session_id | jq -r '.[]'
curl -sS -w "\n" -k https://$ActIP/actifio/api/report/reportapps?sessionid=$session_id
